...
def load():
    try:
        with open("state.pickle", "rb") as f:
            state = pickle.load(f)
        with open("agent_colour.pickle", "rb") as f:
            agent_colour = pickle.load(f)
        return state, agent_colour
    except FileNotFoundError:
        print("No saved game state found. Please start a new game with the -begin flag.")
        sys.exit(1)
...
def ai_move():
    # Reset the environment to initialize game object and current agent
    env.reset()
    ...
    # Also set on the unwrapped base env so current_agent is visible to get_valid_actions
    env.unwrapped.current_agent = agent_colour

    #load the agent in the environment
    network = torch.load('./model.pth', weights_only=False)
